...
export default function Login() {
  ...
  const [verifiedMessageShown, setVerifiedMessageShown] = useState(false); // State to track if verified message is shown or not
  // Function
    ...
    try {
      if (email.length > 0 && password.length > 0) {
        ...
        if (user.user.emailVerified) {
          if (!verifiedMessageShown) {
            alert("You Are Verified");
            setVerifiedMessageShown(true); // Set verified message shown to true
          }
